 ...
 */
static int load_block(ulong block, uchar *dst, int len)
{
	/* We may want to get the final block from the previous set */
	ulong offset = ((int)block - 1) * len + TftpBlockWrapOffset;
	ulong tosend = len;

	tosend = min((NetBootFileXferSize > offset ? NetBootFileXferSize - offset : 0), tosend);
	if(tosend > 0)
		memcpy(dst, (void *)(save_addr + offset), tosend);
...
#ifdef CONFIG_CMD_TFTPPUT
static void icmp_handler(unsigned type, unsigned code)
{
...
TftpHandler (uchar * pkt, unsigned dest, unsigned src, unsigned len)
{
	...
	if (TftpState != STATE_SEND_RRQ && src != TftpServerPort &&
