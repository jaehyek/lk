...
static void NetClearHandler(void)
{
	NetSetHandler(NULL);
	NetSetTimeout(0, NULL);
}

static void NetCleanupLoop(void)
{
	NetClearHandler();
}
...
static void PingReceive(IP_t *ip, Ethernet_t *et, int len)
{
...
static void receive_icmp(IP_t *ip, Ethernet_t *et, int len)
{
	ICMP_t *icmph = (ICMP_t *)&(ip->udp_src);
